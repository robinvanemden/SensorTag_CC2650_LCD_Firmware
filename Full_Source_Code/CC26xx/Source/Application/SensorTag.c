...
static void MysensorTag_updateAdvertisingData(void);
...
static void MysensorTag_updateAdvertisingData(void)
{
	...
	// set parameter
	bStatus_t st1 =  Humidity_setParameter(SENSOR_CONF,1,&HumiditySensorON); // turn on
	SensorTagHum_processCharChangeEvt(SENSOR_CONF); // enable humidity sensing

	// get parameter
	bStatus_t st2 =  Humidity_getParameter(SENSOR_PERI, &period);
	bStatus_t st3 =  Humidity_getParameter(SENSOR_CONF, &config);
	bStatus_t st4 =  Humidity_getParameter(SENSOR_DATA, &HumRawData);
