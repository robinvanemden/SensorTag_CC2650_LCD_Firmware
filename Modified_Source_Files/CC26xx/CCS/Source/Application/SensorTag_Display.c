...
void SensorTag_displayBatteryVoltage(void)
{
  ...
  sec = Clock_getTicks();
  xorsec = bitXor(sec,key);

  itoa(sec, bufsec, 10);
  itoa(xorsec, bufxorsec, 10);

  int len =  strlen(bufsec);
  const char *last_four = &bufsec[len-4];
  devpkLcdText(last_four, 8, 2);

  int len2 =  strlen(bufxorsec);
  const char *last_four2 = &bufxorsec[len-4];
  devpkLcdText(last_four2, 9, 2);
